<?php
	...
	****************************************************************** */
	...
	$con = getdb();
	$Sql_Select = "SELECT n.* FROM notification_receiver n WHERE n.active = 1 AND DATE(n.end_date) >= '".date("Y-m-d")."' AND n.push_notification =1 AND n.contact_no = '".$DWUserContact."' ORDER BY n.created DESC";
	$Rst_Select = mysqli_query($con,$Sql_Select);

	$Sql_Count = "SELECT n.* FROM notification_receiver n WHERE n.notification_read = 0 AND n.push_notification = 1";
