    ...
    ****************************************************************** */
	...
	function getRandVoucher($VoucherCode="") {
        ...
        if (mysqli_num_rows($Rst_Select) > 0) {//Maksudnya wujud voucher untuk bulan tu
            ...
            $SqlInfo_SelectCust = "SELECT c.* FROM data_customer c WHERE MONTH(c.date_of_birth) = MONTH('".date("Y-m-d")."') AND DAY(c.date_of_birth) = DAY('".date("Y-m-d")."') AND c.active > 0 ";
            ...
            if (mysqli_num_rows($RstInfo_SelectCust) > 0) {//maksudnya ada user yg hari jadi dia pada hari itu
                while ($InfoDataCust = mysqli_fetch_assoc($RstInfo_SelectCust)) {
                    ...
                    $SqlVou_Insert = "INSERT INTO data_voucher_cust (voucher_id, contact_no, voucher_code, barcode, trxvalue, updated, created, claim_date, voucher_status) VALUES ('".$InfoDataBirthday["id"]."', '".$InfoDataCust["contact_no"]."', '".$DWVoucherCode."', '".$barcode_img_name."', '".$InfoDataBirthday["voucher_value"]."', NOW(), NOW(), NOW(), 1)  ";
