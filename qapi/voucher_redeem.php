    	...
    	for ($i=0; $i<count($DWVoucherData); $i++) {
			...
			if ( $DWCustID > 0 ) {
				...
				if ( $DWVoucherData[$i]->code != "" ) {
					//echo "<br />".
					$SqlVou_Select = "SELECT v.id, v.voucher_code, v.customer_id, i.info_validity_from, i.info_validity_to, i.info_value FROM `data_voucher` v INNER JOIN `data_voucher_info` i ON (i.id=v.info_id AND i.active=1) WHERE v.voucher_code = '".$DWVoucherData[$i]->code."' AND v.active = 1 ";
			    	$RstVou_Select = mysql_query($SqlVou_Select);
			    	if ( mysql_num_rows($RstVou_Select) > 0 ) {

			    		$DWVouID = mysql_result($RstVou_Select, 0, "id");
			    		$DWCustID = mysql_result($RstVou_Select, 0, "customer_id");

			    		if ($DWCustID == "0") {
			    			//echo "<br />".
				    		$SqlCust_Update = "UPDATE data_voucher SET customer_id = '".$DWCustID."', active = 2, trxvalue = '".$DWVoucherData[$i]->value."', trxdate = '".date("Y-m-d", strtotime($DWVoucherData[$i]->trxdate))."', trxnum = '".$DWVoucherData[$i]->trxnum."' WHERE id = '".$DWVouID."' ";
			    			$RstCust_Update = mysql_query($SqlCust_Update);
			    		} else {
			    			//echo "<br />".
				    		$SqlCust_Update = "UPDATE data_voucher SET active = 2, trxvalue = '".$DWVoucherData[$i]->value."', trxdate = '".date("Y-m-d", strtotime($DWVoucherData[$i]->trxdate))."', trxnum = '".$DWVoucherData[$i]->trxnum."' WHERE id = '".$DWVouID."' ";
			    			$RstCust_Update = mysql_query($SqlCust_Update);
			    		}
			    		

		        	    $SqlSales_Insert = "INSERT INTO `data_sales_transaction` (sbclient_id, trxdate, trxnum, trxtotal, updated, created, active) VALUES ('".$DWVoucherData[$i]->id."', '".date("Y-m-d", strtotime($DWVoucherData[$i]->trxdate))."', '".$DWVoucherData[$i]->trxnum."', '".$DWVoucherData[$i]->value."', NOW(), NOW(), 5) ";
