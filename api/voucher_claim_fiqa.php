<?php
	...
	****************************************************************** */
	...
	$fp = fopen('logs/voucher_claim_fiqa.txt', 'a');
	...
	****************************************************************** */
	...
	{
		// $sql = "SELECT SUM(points) as pts FROM data_customer_points_test WHERE contact_no = '".$contact_no."' AND active = 1 and point_type='1' and date(transaction_date) <='2021-01-26'";
	...
	{
		// $sql = "SELECT SUM(points) as pts FROM data_customer_points_test WHERE contact_no = '".$contact_no."' AND active = 1 and point_type='0' and date(transaction_date) <='2021-01-26'";
	...
	if ( $DWUserContact != "" && $DWVoucherInfoID > 0 ) {
		...
		echo $SqlInfo_Select = "SELECT v.* FROM data_voucher v WHERE v.id = '".$DWVoucherInfoID."' AND DATE(v.start_date) <= '".date("Y-m-d")."' AND DATE(v.end_date) >= '".date("Y-m-d")."' AND v.exchange_point <= '".$DWCustPointSingle."' AND v.active = 1 ";
