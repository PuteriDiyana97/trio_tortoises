<?php
	...
	****************************************************************** */
	...
	$fp = fopen('logs/join_account_add_verification.txt', 'a');
	...
	****************************************************************** */
	...
	// $Sql_Count = "SELECT d.* FROM data_customer d WHERE d.contact_no = '".$DWUserContact."' AND d.parent_contact_no = d.contact_no"; //NO PARENT
	// $Sql_Count = "SELECT d.* FROM data_customer d WHERE d.contact_no = '".$DWUserContact."' AND d.parent_contact_no != d.contact_no"; //PARENT EXIST
	...
	$Rst_Select = mysqli_query($con,$Sql_Select);
		...
		if ( mysqli_num_rows($Rst_Select) > 0 ) {
			...
			$Sql_Update = "UPDATE data_customer SET group_id = '".$join_group_id."', firsttime = NOW(),active = 1 WHERE contact_no = '".$DWUserContactChild."'";
