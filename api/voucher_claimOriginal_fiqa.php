<?php
    ...
    ****************************************************************** */
    ...
    $DWUserContact = ReturnValidContact('60108418888');
    $DWVoucherInfoID = '48';//$_REQUEST["voucher_id"];
    $con = getdb();
    ...
    {
        $sql = "SELECT SUM(points) as pts FROM data_customer_points_test WHERE contact_no = '".$contact_no."' AND active = 1 and point_type='1'";
        $con = getdb();
        $q = mysqli_query($con,$sql);
        $col_pts = 0;
        if (mysqli_num_rows($q) > 0) {
    ...
    {
        $sql = "SELECT SUM(points) as pts FROM data_customer_points_test WHERE contact_no = '".$contact_no."' AND active = 1 and point_type='0'";
        $con = getdb();
        $q = mysqli_query($con,$sql);
        $col_pts = 0;
        if (mysqli_num_rows($q) > 0) {
    ...
    function getIndividual_point($contact_no)
    {
        ...
        echo $individual_add_point;

        echo '<br>';

        echo $individual_deduct_point;
            ...
            if (mysqli_num_rows($RstInfo_SelectGroupPoint) > 0) { //means group cukup point so boleh proceed, then kena cari siapa highest
                ...
                if (mysqli_num_rows($RstInfo_SelectContact) > 0) { //means group exist so boleh proceed
                    ...
                    $sql_get_sum_totall = "SELECT data_customer_points.contact_no, SUM(data_customer_points.points) as total_sum_point
                                            ...
                                            HAVING total_sum_point > '".$remaining_pending."'
                                            ORDER BY total_sum_point DESC, data_customer.created ASC
                                            LIMIT 1";
                    $RstInfo_Search = mysqli_query($con,$sql_get_sum_totall);

                    $InfoDataCustomer = mysqli_fetch_assoc($RstInfo_Search);
                    $highest_point_contactno = $InfoDataCustomer["contact_no"];
                    $lender_point = getIndividual_point($highest_point_contactno);

                    
                    //////PART NAK CLAIM VOUCHER///////////////////
                    $con = getdb();
                    $DWVoucherCode = "";
                    $DWVoucherCode = getRandVoucher();
                    // echo $DWVoucherCode;
                    ...
                    // $SqlVou_Insert = "INSERT INTO data_voucher_cust (voucher_id, contact_no, voucher_code, trxvalue, updated, created, claim_date, voucher_status) VALUES ('".$InfoDataVoucher["id"]."', '".$DWUserContact."', '".$DWVoucherCode."', '".$InfoDataVoucher["voucher_value"]."', NOW(), NOW(), NOW(), 1)  ";
                    // $RstVou_Insert = mysqli_query($con,$SqlVou_Insert);
                    ...
                    if (mysqli_num_rows($Rst_SelectParent) > 0 ) { //means dah ada group
                        $check_group_id = find_group_id($DWUserContact);
                        // $SqlPoint_InsertFirst = "INSERT INTO data_customer_points (group_id, contact_no, points, point_type, description, transaction_date, created, active) VALUES ('".$check_group_id."', '".$DWUserContact."', '".$DWCustPointSingle."', 0, 'Voucher Claim', NOW(), NOW(), 1)  ";
                        // $RstPoint_InsertFirst = mysqli_query($con,$SqlPoint_InsertFirst);

                        // $SqlPoint_InsertSecond = "INSERT INTO data_customer_points (group_id, contact_no, points, point_type, description, transaction_date, created, active) VALUES ('".$check_group_id."', '".$highest_point_contactno."', '".$remaining_pending."', 0, 'Voucher Claim', NOW(), NOW(), 1)  ";
                        // $RstPoint_InsertSecond = mysqli_query($con,$SqlPoint_InsertSecond);
                    }
                    else{ //takda group
                        $MainData["status"] = "2";
