	...
	****************************************************************** */
	...
	/* ******************************************************************
		...
		- POINT_LIST = Point Info (Array)
			...
			- LOCATION = Trasaction Location
			- AMOUNT = Trasaction Amount
			- POINT = Point Earn from Trasaction
	...
	if ( $_REQUEST["contactno"] != "" ) {
		...
		while ($RowData = mysql_fetch_assoc($Rst_Select)) {
			...
			while ($TraData = mysql_fetch_assoc($RstTra_Select)) {
				unset($TrasactionData);
				$TrasactionData["DATE"] = date("d M Y h:i A", strtotime($TraData["created"]));// "01 MAY 2019 03:42 PM";
				$TrasactionData["LOCATION"] = OutletDisplay($TraData["trxnum"]);//"Sri Rampai";//$TraData["trxnum"];
				$TrasactionData["AMOUNT"] = "RM ".$TraData["trxtotal"];
				$TrasactionData["POINT"] = $TraData["trxpoint_desc"];//number_format($TraData["trxtotal"])." pts";
				$MainData["POINT_LIST"][] = $TrasactionData;
			}
