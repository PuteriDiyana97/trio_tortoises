<?php
	...
	****************************************************************** */
	...
	$con = getdb();
	$Sql_Select = "SELECT d.* FROM data_customer d WHERE d.contact_no = '".$DWUserContact."' AND d.otp = '".$DWOtp."' AND d.active =2 LIMIT 1";
	...
	$Rst_Select = mysqli_query($con,$Sql_Select);
		...
		if ( mysqli_num_rows($Rst_Select) > 0 ) {
			...
			$Sql_Update = "UPDATE data_customer SET contact_no = '".$DWUserContact."', otp = '".$DWOtp."', firsttime = NOW(),active = 1 WHERE id = '".$DWID."'";
