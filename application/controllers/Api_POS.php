		...
		if ( count($DWSalesData) > 0 ) {
			for ($i=0; $i<count($DWSalesData); $i++) {
				...
				if ( !empty($DWSalesData[$i]->trxnum) ) {
					...
					$terminal = isset($DWSalesData[$i]->terminal) ? $DWSalesData[$i]->terminal : '';
					...
					if ( !empty($data_st) ) {
						...
						$update_sales = $this->db->where('id',$data_st->id)->update('data_sales_transaction', $data);
						...
						$db = $this->db;
						$db->select('*');
						$db->where('description', 'Sales for '.$DWSalesData[$i]->trxnum);
						$point_cust = $db->get('data_customer_points')->row();
						...
						if(empty($point_cust)){
							...
							$insert_data_customer_points = $this->db->where('id',$point_cust->id)->update('data_customer_points', $data_point);
						...
						$APIData["status"] = 1;
						$APIData["status_message"] = "Transaction Updated";
		...
		if ( count($DWVoidData) > 0 ) {
			for ($i=0; $i<count($DWVoidData); $i++) {
				if ( !empty($DWVoidData[$i]->trxnum) ) {
					...
					$terminal = isset($DWVoidData[$i]->terminal) ? $DWVoidData[$i]->terminal : '';
					...
					if ( !empty($void)) {
						...
						$update_sales = $this->db->where('id',$void->id)->update('data_sales_transaction', $data);
						$APIData["status"] = 1;
						$APIData["status_message"] = "Transaction Updated";
