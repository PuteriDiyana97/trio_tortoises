...
class Birthday_voucher_Controller extends My_Controller 
{
	public function __construct()
	{
		parent::__construct();
		$this->load->model('voucher/Birthday_vouchers');
	}
	...
	public function index()
	{
        ...
        $this->data = $data;
        $this->middle = 'voucher/birthday_voucher';
        $this->layout();
	}
	...
	public function list1()
	{
		...
		$data = array();
	        ...
	        $no = $_POST['start'];
			$data = array();
			$list = $this->Birthday_vouchers->get_datatables();
	 ...
	 //pre($field); die();
	        $output = array(
	            "draw" => $_POST['draw'],
	            "recordsTotal" => $this->Birthday_vouchers->count_all(),
	            "recordsFiltered" => $this->Birthday_vouchers->count_filtered(),
	            "data" => $data,
	        );
	...
	public function create()
    {
        ...
        $this->data = $data;
        $this->middle = 'voucher/add_birthday_voucher';
        $this->layout();
    }
	...
	{	
		...
		$year = date('Y');
		$date_in_month_arr = getDaysInYearMonth($year,$month,'Y-m-d');

		$first_start_date = reset($date_in_month_arr);
		$last_end_date = end($date_in_month_arr);
        ...
        echo json_encode($data);
        die();
        // redirect('birthday_voucher/create');
		}
        ...
        $status = $this->Birthday_vouchers->insert($form);
	...
	public function details()
	{
		...
		$data = $this->Birthday_vouchers->read_data($selected_id);
	...
	public function update()
	{
		...
		//checking voucher image after used
		$uploadOk = 1; //init variable
		
		$target_dir = "assets/upload_files/voucher/";
		$target_file2 = $target_dir . basename($voucher_img_after_new_name);
        ...
        $status = $this->Birthday_vouchers->update_data($selected_id, $form);
	...
	public function delete()
	{
		...
		$status = $this->Birthday_vouchers->delete_data($selected_id);
        if ($status) {
            $data = [
                'status' => 1,
