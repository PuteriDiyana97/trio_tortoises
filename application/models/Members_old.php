...
class Members extends CI_Model 
{
	...
	// ======================= DATATABLE SERVER SIDE PROCESSING =========================
     ...
     var $column_order = array(null, 'a.name', 'a.email', 'a.contact_no', 'a.state', 'a.country', 'e.current_points', 'b.total_points', 'a.active', null); // field yang ada di table
    var $column_search = array('a.name', 'a.email', 'a.contact_no', 'a.state', 'a.country', 'a.active'); // field yang diizinkan untuk pencarian 
    var $order = array('UPPER(a.name)' => 'ASC'); // default order

    private function _get_datatables_query()
    {
        $filter_name = $this->input->post('filter_name');
        $filter_member_status = $this->input->post('filter_member_status');
        $filter_country = $this->input->post('filter_country');
        $filter_contact = $this->input->post('filter_contact');

        $db = $this->db;
        $db->select('a.id, a.name, a.email, a.contact_no, a.state, a.country, a.active, e.sum_points, d.deduct_points, b.total_points');
